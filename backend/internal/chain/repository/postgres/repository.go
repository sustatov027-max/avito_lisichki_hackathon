...
func (r *ChainRepository) ProcessDecision(
	...
	// 1. Блокировка цепочки через advisory lock, чтобы исключить race condition между участниками
	...
	// 3. Получение шага пользователя и проверка, что решение еще не было принято
...
func (r *ChainRepository) processReject(ctx context.Context, tx pgx.Tx, chainID, stepID uuid.UUID) (string, error) {
	// Отмечаем шаг как rejected (FALSE)
	_, err := tx.Exec(ctx, `
	...
	// Переводим цепочку в статус 'rejected'
	_, err = tx.Exec(ctx, `
...
func (r *ChainRepository) processAccept(ctx context.Context, tx pgx.Tx, chainID, stepID uuid.UUID) (string, error) {
	// Отмечаем шаг как accepted (TRUE)
	_, err := tx.Exec(ctx, `
		UPDATE exchange_chain_steps 
	...
	// Проверяем, есть ли еще незавершенные шаги (NULL или FALSE)
	...
	// Если остались необработанные шаги, цепочка остается в статусе 'proposed'
	if unacceptedCount > 0 {
		return "proposed", nil
	}

	// Если ВСЕ приняли — завершаем цепочку (Hard-Lock товаров и инвалидация пересекающихся цепочек)
...
func (r *ChainRepository) finalizeAcceptedChain(ctx context.Context, tx pgx.Tx, chainID uuid.UUID) error {
	...
	// 2. Статус всех товаров в цепочке -> 'reserved' (Hard-Lock)
	...
	// 3. Перевод всех параллельных цепочек в статус 'invalidated'
	_, err = tx.Exec(ctx, `
		UPDATE exchange_chains 
